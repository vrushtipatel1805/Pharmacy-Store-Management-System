...
public class PharmacyStore3 {
    ...
    public class LowStockMinHeap {
        ...
        public LowStockMinHeap(int cap) {
            heap = new Item[cap];
        }
    ...
    private void recordPurchase() {
        try (Connection conn = connectDB()) {
                ...
                if (expiryDate.isBefore(LocalDate.now())) {
                    System.out.println("❌ Medicine already expired (" + expiryDate + "). Not added.");
                    return;
                }
    ...
    private void salesReportBetweenDates() {
        ...
        try (Connection conn = connectDB()) {
            ...
            // 1️⃣ Total Sales
            String sqlTotal = "SELECT IFNULL(SUM(final_amount),0) AS total_sales " +
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlTotal)) {
                ...
                if (rs.next()) {
                    ...
                    System.out.printf("TOTAL SALES AFTER DISCOUNT AND TAX (₹): %.2f%n", totalSales);
                    System.out.println("=====================================");
                }
            }

            // 2️⃣ Per-Medicine Sales
            String sqlMedicines =
                    "SELECT m.id, m.name, SUM(bi.quantity) AS total_qty, " +
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlMedicines)) {
                ps.setDate(1, java.sql.Date.valueOf(start));
                ps.setDate(2, java.sql.Date.valueOf(end));
                ResultSet rs = ps.executeQuery();

                System.out.println("\n--- Per-Medicine Sales ---");
                System.out.printf("%-5s %-30s %-10s %-12s%n",
                        "ID", "Medicine", "Qty", "Total (₹)");
                System.out.println("==================================================");
    ...
    private void exportBillToText(int billId) {
        String folder = "bills";
        java.io.File dir = new java.io.File(folder);
        if (!dir.exists()) dir.mkdirs(); // create folder if not exist

        String fileName = folder + "/Bill_" + billId + ".txt";

        String billQuery = "SELECT b.bill_number, b.created_at, b.total_amount, " +
                "b.discount, b.tax_amount, b.final_amount, " +
                "c.name AS customer_name " +
                "FROM bills b " +
                "LEFT JOIN customers c ON b.customer_id = c.id " +
                "WHERE b.id = ?";

        String itemsQuery = "SELECT m.name, bi.quantity, bi.unit_price " +
                "FROM bill_items bi " +
                "JOIN medicines m ON bi.medicine_id = m.id " +
                "WHERE bi.bill_id = ?";
        ...
        try (Connection conn = connectDB();
            ...
            String billNumber = billRs.getString("bill_number");
            String customer = billRs.getString("customer_name");
            double total = billRs.getDouble("total_amount");
            double discount = billRs.getDouble("discount");
            double tax = billRs.getDouble("tax_amount");
            double finalAmt = billRs.getDouble("final_amount");
            java.sql.Timestamp createdAt = billRs.getTimestamp("created_at");

            try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
                writer.write("💊 Pharmacy Store Bill\n");
                writer.write("Bill No: " + billNumber + "\n");
                writer.write("Date: " + createdAt.toLocalDateTime() + "\n");
                writer.write("Customer: " + (customer != null ? customer : "Walk-in") + "\n");
                writer.write("------------------------------------------------------\n");
                writer.write("\tMedicine \t\tQty \t\tUnit Price\n");

                itemsPs.setInt(1, billId);
                ResultSet itemsRs = itemsPs.executeQuery();

                while (itemsRs.next()) {
                    writer.write(" "+itemsRs.getString("name") + "\t" +
                ...
                writer.write("Tax (18%): ₹" + tax + "\n");
                writer.write("Final Amount: ₹" + finalAmt + "\n");
            }

            System.out.println("✅ Bill exported to TXT: " + fileName);
            ...
            switch (ch) {
                case 1 -> {
                    ...
                    while (o) {
                        ...
                        System.out.println("0. go to main manu");
                ...
                case 4 -> {
                    ...
                    while (o) {
                        ...
                        switch (r) {
                            case 1 -> {salesReportBetweenDates();}
                            case 2 -> {expiryReportWithHeap();}
    ...
    public static void main(String[] args) {
        PharmacyStore3 app = new PharmacyStore3();
        app.startMenu();
    }
}